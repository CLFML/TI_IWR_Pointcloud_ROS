...
/**
 * @brief Represents a complete radar frame with various matrices and point
 * clouds.
 */
struct radar_frame_t {
  ...
  std::vector<std::vector<float>>
      target_list; /**< List of tracked targets [target][9 values] */
...
struct packet_parser_cfg_t {
  ...
  std::function<void(const radar_frame_t &frame)> cbfunc =
      nullptr; /**< Callback for parsed radar frames */
};
...
private:
  ...
  bool _serial_ports_opened =
      false; /**< Indicates if serial ports were successfully opened */
  ...
  static constexpr uint32_t _speed_of_light =
      299792458; /**< Speed of light in m/s */
  static constexpr std::array<uint8_t, 8> _magic_word = {
      0x02, 0x01, 0x04, 0x03,
      0x06, 0x05, 0x08, 0x07}; /**< Magic word for packet start */

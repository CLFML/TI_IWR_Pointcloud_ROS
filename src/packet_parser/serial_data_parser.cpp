...
size_t PacketParser::find_magic_word(const std::vector<uint8_t>& buf, size_t offset) {
    if (buf.size() < offset + _magic_word.size()) return std::string::npos;

    auto it = std::search(buf.begin() + offset, buf.end(),
                          _magic_word.begin(), _magic_word.end());
...
void PacketParser::process_buffer() {
    while (true) {
        ...
        if (_buffer.size() < start + 36) return;

        uint32_t packet_length = 0;
        std::memcpy(&packet_length, &_buffer[start + 12], sizeof(uint32_t));

        if (packet_length < 36 || packet_length > 65536) {
            std::cerr << "Suspicious packet length: " << packet_length << ", skipping magic.\n";
            _read_offset = start + _magic_word.size();
            continue;
        }

        if (_buffer.size() < start + packet_length) return;

        std::vector<uint8_t> frame(_buffer.begin() + start, _buffer.begin() + start + packet_length);
